...
public class SpawnersCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.executes(context -> {
            ...
            slot = giveXpOrbSpawner(slot, "XP ORB FLOOD", Formatting.GREEN, 32767, 1, 1, 1, 10000, 64000, 640, 160);
            ...
            slot = giveShulkerBulletSpawner(slot, "SHULKER BULLET STORM", Formatting.LIGHT_PURPLE, 1, 1, 1, 10000, 64000, 640, 160);
            slot = giveTridentSpawner(slot, "TRIDENT STORM", Formatting.AQUA, 1, 1, 1, 8000, 64000, 640, 160);
            slot = giveFallingBlockSpawner(slot, "FALLING BEDROCK PRISON", Formatting.DARK_GRAY, "minecraft:bedrock", true, 1000, 100.0f, 1, 1, 1, 8000, 64000, 640, 100);
            slot = giveFallingBlockSpawner(slot, "WATER FLOOD", Formatting.BLUE, "minecraft:water", false, 0, 0, 1, 1, 1, 8000, 64000, 640, 100);
            slot = giveTntSpawner(slot, "GLOCK 19", Formatting.RED, 60, 1, 1, 1, 8000, 64000, 640, 160);
            slot = giveTntSpawner(slot, "TNT SHOTGUN", Formatting.DARK_RED, 5, 1, 1, 1, 8000, 64000, 640, 100);
            slot = giveTntSpawner(slot, "INSTANT TNT NUKE", Formatting.DARK_RED, 1, 1, 1, 1, 8000, 64000, 640, 160);
            slot = giveBadOmenSpawner(slot);
            slot = giveFallingBlockSpawner(slot, "REDSTONE ACTIVATOR", Formatting.RED, "minecraft:redstone_block", false, 0, 0, 1, 1, 1, 100, 64000, 640, 50);
            slot = giveEvokerFangSpawner(slot, "EVOKER FANG CIRCLE", Formatting.DARK_AQUA, 1, 1, 1, 5000, 64000, 640, 100);
            slot = giveWindChargeSpawner(slot, "WIND CHARGE CHAOS", Formatting.WHITE, 1, 1, 1, 8000, 64000, 640, 160);
            slot = giveBreezeSpawner(slot, "BREEZE ARMY", Formatting.AQUA, 1, 1, 1, 500, 6400, 640, 100);
            slot = giveEndCrystalSpawner(slot, "END CRYSTAL NUKE", Formatting.LIGHT_PURPLE, 1, 1, 1, 1000, 64000, 640, 100);
    ...
    private int giveEvokerFangSpawner(int slot, String name, Formatting color, int delay, int minDelay, int maxDelay, int spawnCount, int maxNearby, int playerRange, int spawnRange) {
        NbtCompound extra = new NbtCompound();
        extra.putInt("Warmup", 0);
        return giveSpawner(slot, name, color, "minecraft:evoker_fangs", extra, delay, minDelay, maxDelay, spawnCount, maxNearby, playerRange, spawnRange);
    }

    private int giveWindChargeSpawner(int slot, String name, Formatting color, int delay, int minDelay, int maxDelay, int spawnCount, int maxNearby, int playerRange, int spawnRange) {
        if (slot >= 36) return slot;
        try {
            ItemStack spawner = new ItemStack(Items.SPAWNER, 64);
            NbtCompound nbt = new NbtCompound();
            ...
            NbtCompound spawnData = new NbtCompound();
            NbtCompound entity = new NbtCompound();
            entity.putString("id", "minecraft:wind_charge");
            NbtList power = new NbtList();
            power.add(NbtDouble.of(0.0));
            power.add(NbtDouble.of(-1.0));
            power.add(NbtDouble.of(0.0));
            ...
            error("Failed wind charge spawner: " + e.getMessage());
            return slot;
        }
    }

    private int giveBreezeSpawner(int slot, String name, Formatting color, int delay, int minDelay, int maxDelay, int spawnCount, int maxNearby, int playerRange, int spawnRange) {
        return giveSpawner(slot, name, color, "minecraft:breeze", null, delay, minDelay, maxDelay, spawnCount, maxNearby, playerRange, spawnRange);
    }

    private int giveEndCrystalSpawner(int slot, String name, Formatting color, int delay, int minDelay, int maxDelay, int spawnCount, int maxNearby, int playerRange, int spawnRange) {
        NbtCompound extra = new NbtCompound();
        extra.putBoolean("ShowBottom", false);
        return giveSpawner(slot, name, color, "minecraft:end_crystal", extra, delay, minDelay, maxDelay, spawnCount, maxNearby, playerRange, spawnRange);
    }
}
