...
public class ServerCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        // Added players subcommand with fixes for 1.21.10
        builder.then(literal("players").executes(ctx -> {
            ...
            if (playerList.isEmpty()) {
                info("No players found on the server.");
                return SINGLE_SUCCESS;
            }
            
            info("Players on server (%d):", playerList.size());
            ...
            for (var playerEntry : playerList) {
                ...
                // FIX: .getId() -> .id()
                String uuid = playerEntry.getProfile().id().toString();
                
                String distance = "Unknown";
                // FIX: .getId() -> .id()
                var playerEntity = mc.world.getPlayerByUuid(playerEntry.getProfile().id());
                if (playerEntity != null && mc.player != null) {
                    // FIX: Manual Vec3d construction
                    Vec3d playerPos = new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ());
                    Vec3d targetPos = new Vec3d(playerEntity.getX(), playerEntity.getY(), playerEntity.getZ());
                    
                    double distanceValue = playerPos.distanceTo(targetPos);
                    distance = String.format("%.1f blocks away", distanceValue);
                }
                
                info("%s, %s, %s", playerName, uuid, distance);
            }
    ...
    private void basicInfo() {
        ...
        // 1.21.11-safe: no more getLocalDifficulty on ClientWorld
        info("Difficulty: %s",
    ...
    private void onReadPacket(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof CommandTreeS2CPacket packet) {
            ...
            // FIX: Combined instance methods and static factory
