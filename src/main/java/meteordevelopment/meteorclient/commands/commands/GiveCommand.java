...
public class GiveCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        }).then(argument("number", IntegerArgumentType.integer(1, 127)).executes(context -> {
            if (!mc.player.getAbilities().creativeMode) throw NOT_IN_CREATIVE.create();

            int count = IntegerArgumentType.getInteger(context, "number");
            // Create with check=false to bypass initial validation
            ItemStack item = ItemStackArgumentType.getItemStackArgument(context, "item").createStack(count, false);
            
            // If count exceeds standard limit, force it with NBT
            if (count > item.getMaxCount()) {
                item.set(DataComponentTypes.MAX_STACK_SIZE, count);
            }
            
            giveItem(item);

            return SINGLE_SUCCESS;
        }).then(argument("repeat", IntegerArgumentType.integer(1, 36)).executes(context -> {
            if (!mc.player.getAbilities().creativeMode) throw NOT_IN_CREATIVE.create();

            int count = IntegerArgumentType.getInteger(context, "number");
            ItemStack item = ItemStackArgumentType.getItemStackArgument(context, "item").createStack(count, false);
            
            // Apply stack size fix here as well
            if (count > item.getMaxCount()) {
                item.set(DataComponentTypes.MAX_STACK_SIZE, count);
            }
